...
- **GitHub release binaries go in `.chezmoiexternals/`.** Each recipe places a `<tool>.toml` file in `chezmoi/.chezmoiexternals/`. These are overlaid to `compiled-home/.chezmoiexternals/` with unique names (no conflicts). Files in `.chezmoiexternals/` are always rendered as templates by chezmoi; use `gitHubLatestReleaseAssetURL` and `gitHubLatestRelease` for the URL and archive path. Shell install scripts are for apt packages and tools needing post-install setup only.
